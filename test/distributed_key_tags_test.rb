...
class TestDistributedKeyTags < Test::Unit::TestCase
  ...
  def test_hashes_consistently
    r1 = Tr8dis::Distributed.new ["redis://localhost:#{PORT}/15", *NODES]
    r2 = Tr8dis::Distributed.new ["redis://localhost:#{PORT}/15", *NODES]
    r3 = Tr8dis::Distributed.new ["redis://localhost:#{PORT}/15", *NODES]
  ...
  def test_allows_clustering_of_keys
    r = Tr8dis::Distributed.new(NODES)
  ...
  def test_allows_passing_a_custom_tag_extractor
    r = Tr8dis::Distributed.new(NODES, :tag => /^(.+?):/)
    r.add_node("redis://localhost:#{PORT}/14")
    r.flushdb
